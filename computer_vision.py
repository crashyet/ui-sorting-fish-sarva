...
class SimpleDetector:
    ...
    def load_model(self):
        """Load the Keras model with compatibility fixes"""
        try:
            ...
            # Method 4: Manual fix for groups parameter
            print("🔧 Mencoba perbaikan manual untuk parameter 'groups'...")
            
            # Load model with custom DepthwiseConv2D
            ...
            print(f"✅ Model berhasil dimuat (metode perbaikan manual): {self.model_path}")
            ...
            print(f"❌ Semua metode loading gagal: {e}")
            sys.exit(1)
        ...
        while True:
            ...
            if conf >= self.confidence_threshold:
                if start_time is None:
                    start_time = now  # mulai hitung durasi confident

                elif not detected and (now - start_time) >= 1.0:
                    # ✅ Ikan terdeteksi valid
                    print(f"✅ Ikan terdeteksi: {label} ({conf:.1%})")
                    detected = True  # hanya tampilkan sekali
            else:
                # Jika confidence rendah, reset semuanya
                start_time = None
                detected = False
