...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.grid_columnconfigure(2, weight=2, uniform="a")  # Log Activity column narrower
    ...
    def create_camera_capture_frame(self):
        ...
        self.jenis_deteksi = ctk.CTkLabel(jenis_frame, text="-", font=ctk.CTkFont(size=15, weight="bold"), text_color="#222")
        self.jenis_deteksi.pack(anchor="w")
        ...
        self.berat_deteksi = ctk.CTkLabel(berat_frame, text="0.0 Kg", font=ctk.CTkFont(size=15, weight="bold"), text_color="#222")
        self.berat_deteksi.pack(anchor="w")
    ...
    def create_box_manager_frame(self):
        ...
        self.box_frame.grid(row=0, column=1, sticky="nsew", padx=(10), pady=(30,5))
        self.box_frame.grid_columnconfigure((0,1,2,3,4), weight=1)
        ctk.CTkLabel(self.box_frame, text="Box Manager", font=ctk.CTkFont(size=22, weight="bold"), text_color="#2563eb").grid(row=0, column=0, columnspan=5, sticky="nw", padx=25, pady=(25))
        ...
        for i, h in enumerate(["A","B","C"]):
            ctk.CTkLabel(self.box_frame, text=h, font=ctk.CTkFont(size=16, weight="bold"), text_color="#2563eb").grid(row=1, column=i+1, pady=(5))

        box_size = 100
        self.box_btns = []
        for r in range(3):
            row_btns = []
            ctk.CTkLabel(self.box_frame, text=str(r+1), font=ctk.CTkFont(size=16, weight="bold"), text_color="#2563eb").grid(row=r+2, column=0, padx=(15,0), pady=15, sticky="nsew")
            for c in range(3):
                btn = ctk.CTkButton(self.box_frame, text="0 Kg", width=80, height=box_size, corner_radius=10, fg_color="#e0e7ff", text_color="#222", hover_color="#7eb5fc", font=ctk.CTkFont(size=16, weight="bold"), command=lambda r=r, c=c: self.show_box_alert(r, c))
                btn.grid(row=r+2, column=c+1, padx=10, pady=20, sticky="nsew")
                row_btns.append(btn)
        ...
        self.trash_btn.grid(row=2, column=4, rowspan=3, padx=(10,25), pady=20, sticky="nsew")

    # Velocity Manager
    def create_velocity_manager_frame(self):
        self.accel_frame = ctk.CTkFrame(self, fg_color="white", corner_radius=20)
        self.accel_frame.grid(row=1, column=1, sticky="nsew", padx=(10,10), pady=(10,30))
