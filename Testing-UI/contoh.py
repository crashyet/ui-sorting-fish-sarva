...
class App(ctk.CTk):
    ...
    def create_camera_capture_frame(self):
        ...
        ctk.CTkLabel(waktu_frame, text="08:20:12", font=ctk.CTkFont(size=16, weight="bold"), text_color="#2563eb").pack(anchor="e")
    ...
    def create_info_widgets(self):
        ...
        info_data = [
            ("1408", "Jumlah Pcs Tersortir"),
            ("52.2", "Jumlah Kg Tersortir"),
            ("281", "Jumlah Pcs / jam"),
            ("10.4", "Jumlah Kg / jam"),
        ]
        for i, (val, desc) in enumerate(info_data):
            ...
            ctk.CTkLabel(box, text=val, font=ctk.CTkFont(size=28, weight="bold"), text_color="#2563eb").grid(row=0, column=0, pady=(15,5), sticky="nsew")
    ...
    def show_box_alert(self, row, col):
        ...
        alert.grab_set()
        ctk.CTkLabel(alert, text=msg, font=ctk.CTkFont(size=16, weight="bold"), text_color="#2563eb").pack(pady=30)
        ctk.CTkButton(alert, text="OK", fg_color="#4ade80", hover_color="#22c55e", font=ctk.CTkFont(size=15, weight="bold"), command=alert.destroy).pack(pady=10)
    ...
    def create_box_manager_frame(self):
        ...
        # Matriks box persegi dengan angka di samping kiri
        box_size = 80
        for r in range(3):
            ...
            for c in range(3):
                btn = ctk.CTkButton(self.box_frame, text="5.4 Kg", width=box_size, height=box_size, corner_radius=10, fg_color="#e0e7ff", text_color="#222", hover_color="#7eb5fc", font=ctk.CTkFont(size=16, weight="bold"), command=lambda r=r, c=c: self.show_box_alert(r, c))
                btn.grid(row=r+2, column=c+1, padx=10, pady=10, sticky="nsew")

        # Trash
        trash_btn = ctk.CTkButton(self.box_frame, text="TRASH\n13.3 Kg", corner_radius=10, fg_color="#ef4444", text_color="white", hover_color="#bc1616", font=ctk.CTkFont(size=16, weight="bold"))
        trash_btn.grid(row=2, column=4, rowspan=3, padx=(20,10), pady=10, sticky="nsew")
    ...
    def create_log_activity_frame(self):
        ...
        self.log_textbox.insert("1.0", "[12:31:22|15/07] Ikan A - 1.4kg → A3\n"*27)
        ...
        start_btn = ctk.CTkButton(btn_frame, text="START", corner_radius=10, fg_color="#4ade80", hover_color="#22c55e", font=ctk.CTkFont(size=18, weight="bold"))
        stop_btn = ctk.CTkButton(btn_frame, text="STOP", corner_radius=10, fg_color="#facc15", hover_color="#eab308", font=ctk.CTkFont(size=18, weight="bold"))
        start_btn.grid(row=0, column=0, padx=(10,5), pady=(7,3), sticky="ew", ipady=10)
        stop_btn.grid(row=0, column=1, padx=(5,10), pady=(7,3), sticky="ew", ipady=10)
        # Reset button below
        reset_btn = ctk.CTkButton(btn_frame, text="RESET", corner_radius=10, fg_color="#f87171", hover_color="#ef4444", font=ctk.CTkFont(size=18, weight="bold"), command=self.confirm_reset_log)
    ...
    def confirm_reset_log(self):
        alert = ctk.CTkToplevel(self)
        alert.title("Konfirmasi Reset Log")
        alert.geometry("350x250")
        alert.transient(self)
        alert.grab_set()
        ctk.CTkLabel(alert, text="Apakah Anda yakin ingin mereset log history mesin?", font=ctk.CTkFont(size=16, weight="bold"), text_color="#000000").pack(pady=30)
        btn_frame = ctk.CTkFrame(alert, fg_color="transparent")
        btn_frame.pack(pady=10)
        ctk.CTkButton(btn_frame, text="Ya, Reset", fg_color="#ef4444", hover_color="#b50000", font=ctk.CTkFont(size=15, weight="bold"), command=lambda: self.reset_log_history(alert)).pack(side="left", padx=10)
    ...
    def reset_log_history(self, alert_window):
