...
class App(ctk.CTk):
    ...
    def create_velocity_manager_frame(self):
        ...
        # Ambil data conveyor sebagai dictionary
        ...
        for i in range(4):
            ...
            if val < 0:
                ...
                val = 20
            slider.set(val)
            slider.grid(row=i+1, column=1, padx=(25,10), pady=10, sticky="ew")
            value_label = ctk.CTkLabel(self.accel_frame, text=str(int(slider.get())), font=ctk.CTkFont(size=15, weight="bold"), width=32)
            value_label.grid(row=i+1, column=2, padx=(0,20), pady=10, sticky="w")
    ...
    def update_all_sections(self):
        ...
        print("Tipe box_data:", type(box_data))
        print("Isi box_data:", box_data)
        box_keys = [
            ["A1", "A2", "A3"],
            ["B1", "B2", "B3"],
            ["C1", "C2", "C3"]
    ...
    def reset_box_value(self, row, col, alert_window):
        key = f"{chr(65 + row)}{col + 1}"  # Contoh: A1, B2, dst
    ...
    def open_settings_window(self):
        ...
        # Camera Auto Checkbox with save to settings.json
        ...
        # Helper untuk membuat input range dan menyimpan referensi entry
        self.range_entries = {}
        ...
        # Set entry values from settings
